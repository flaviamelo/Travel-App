...
const getGeoName = async (baseURL, cityName) => {
	const urlToFetch = `${baseURL}${encodeURIComponent(cityName)}&maxRows=1&username=${userName}`;
...
const weatherbitIpi = process.env.WEATHERBIT_KEY;
...
const pixabayIpi = process.env.PIXABAY_KEY;
// Query data Weatherbit - GET request
const getPixabay = async (baseURL, searchTerm) => {
	const urlToFetch = `${baseURL}?key=${pixabayIpi}&q=${encodeURIComponent(searchTerm)}`;
...
app.post('/query', (req, res) => {
	...
	getGeoName(baseURLGeoNames, query).then((geoData) => {
		const { toponymName, lat, lng, countryName, population, countryCode } = geoData.geonames[0];
		let geoNames = {};
		...
		getWeatherbit(baseURLWeatherbit, lastLat, lastLng).then((data) => {
			...
			getPixabay(baseURLPixabay, `${latestGeoData.city} ${latestGeoData.country}`).then((data) => {
				...
				getRestCountries(latestGeoData.country).then((data) => {
					const { name, callingCodes, capital, region, demonym, flag, languages, currencies } = data[0];
