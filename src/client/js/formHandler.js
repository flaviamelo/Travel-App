function handleSubmit(event) {
	...
	instructions.style.display === 'none' ? (buttonShowInstructions.style.display = 'block') : null;
	...
	dateText.length === 10
		? postData('http://localhost:3000/query', formtTextObj).then((data) => {
				getAllData().then((data) => {
					...
					const latestCountryData = data.countryInfo[data.countryInfo.length - 1];
					...
					const tripWeatherDescription = latestWeatherData.weatherPerDay[0].weatherDescription;
